...
var sankey = function() {
  ...
  function manuallySetPos(graph) {
    graph.nodes.forEach(function(node, i) {
      var width = node.x1 - node.x0;
      if (node.posXrel != undefined){
	var theNode;
        graph.nodes.forEach(function(nodea, i) {
          if (nodea.id == node.posXrel){
	    theNode = nodea;
	  }
	});
	node.x0 = theNode.x0;
	...
	node.x1 = node.x0 + width;
      ...
      if (node.posYrel != undefined){
	var theNode;
        graph.nodes.forEach(function(nodea, i) {
          if (nodea.id == node.posYrel){
	    theNode = nodea;
	  }
	});
	node.y1 = theNode.y0;
	node.y0 = node.y1 - height;
      }
      if (node.posYoff != undefined){
	node.y0 += node.posYoff;
	node.y1 += node.posYoff;
      }
      if (node.posY != undefined){
	node.y0 = node.posY;
	node.y1 = node.y0 + height;
      }
      if (node.posYbot != undefined){
	node.y1 = node.posYbot;
	node.y0 = node.y1 - height;
      }
    });    
  }
  ...
  function computeLinkBreadths(graph) {
    graph.nodes.forEach(function(node) {
      ...
      node.sourceLinks.sort(indexDiff);
      node.targetLinks.sort(indexDiff);
    });
    graph.nodes.forEach(function(node) {
