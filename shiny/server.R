...
shinyServer(function(input, output) {
    ...
    output$plot <- renderPlot({
        if (input$metric == "Lawsuit trends"){
            ...
            if (input$dtzip != 'Any'){
                ...
                for (indexno in seq_along(years)){
                    tenure.renter <- acs_sort(get_acs(geography="tract", year=years[indexno], variables="B25003_003", state="PA", county="Philadelphia County", geometry=FALSE))
                    tenure.total <- acs_sort(get_acs(geography="tract", year=years[indexno], variables="B25003_001", state="PA", county="Philadelphia County", geometry=FALSE))
    ...
    output$mymap <- renderLeaflet({
        ...
        if(input$metric == "Eviction process" || input$metric == "Lawsuit trends" || input$metric == "Rental unit trends"){
            ...
            else if (input$metric == "Rented vs. owned change" || input$metric == "Lawsuit rate change" || input$metric == "Ratio of licenses to units change"){
                tenure.renter.upper <- acs_sort(get_acs(geography="tract", year=acs.year.upper, variables="B25003_003", state="PA", county="Philadelphia County", geometry=TRUE))
                tenure.total.upper <- acs_sort(get_acs(geography="tract", year=acs.year.upper, variables="B25003_001", state="PA", county="Philadelphia County", geometry=TRUE))
                tenure.renter.lower <- acs_sort(get_acs(geography="tract", year=acs.year.lower, variables="B25003_003", state="PA", county="Philadelphia County", geometry=TRUE))
                tenure.total.lower <- acs_sort(get_acs(geography="tract", year=acs.year.lower, variables="B25003_001", state="PA", county="Philadelphia County", geometry=TRUE))
            }
            else{
                tenure.renter <- acs_sort(get_acs(geography="tract", year=acs.year, variables="B25003_003", state="PA", county="Philadelphia County", geometry=TRUE))
                tenure.total <- acs_sort(get_acs(geography="tract", year=acs.year, variables="B25003_001", state="PA", county="Philadelphia County", geometry=TRUE))
            }
